<script setup lang='ts'>
  ...
  import {
    useCopyCode
  } from './hooks/useCopyCode'
  import {
    useUsingContext
  } from './hooks/useUsingContext'
  ...
  const openLongReply =
    import.meta.env.VITE_GLOB_OPEN_LONG_REPLY === 'true'
      ...
      const fetchChatAPIOnce = async () => {
        await fetchChatAPIProcess < Chat.ConversationResponse > ({
          ...
          onDownloadProgress: ({
            ...
            if (rep.message === 'Unauthorized') {
              ms.error("登录已过期,请重新登录");
              router.replace({
                path: "/login"
              });
              return;
  ...
  const searchOptions = computed(() => {
    if (prompt.value.startsWith('/')) {
      return promptTemplate.value.filter((item: {
        key: string
      }) => item.key.toLowerCase().includes(prompt.value.substring(1).toLowerCase())).map((obj: {
        value: any
      }) => {
        return {
          label: obj.value,
